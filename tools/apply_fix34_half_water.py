...
# Reserve one aux bit for fresh-water volume. The bit is removed from random
# entropy so only explicit water operations may set it.
replace_once(
...
# Fine movement owns half-volume conservation. Fresh water spreads by splitting
# one full cell into two halves; adjacent halves merge back into one full cell.
replace_once(
    "shaders/move.comp",
    "const uint AUX_MOVED = 0x01000000u;\n"
    "const uint AUX_STATE_MASK = 0x000000ffu;\n",
...
replace_once(
    ...
    "        // A ledge does not release until the edge holds one full cell and\n"
    "        // the immediately trailing cell contributes at least one half-unit.\n"
...
replace_once(
    ...
    "        return;\n"
    "    }\n\n"
    "    // The final four horizontal passes are half-water-only. Full water and\n"
    "    // every other material keep the original pass count.\n"
...
# Chemistry may preserve a half bit only while an existing fresh-water cell
# remains fresh water. Reactions and phase/material conversions always produce
# a full water cell or a non-water material.
replace_once(
...
# Half cells remain fine-grained so macro transfers cannot hide or duplicate
# their one-half-unit state.
replace_once(
...
# Faint water is a 50% visual blend of the normal water color and world
# background; two halves merging restores the normal full color.
replace_once(
...
# Four ordinary horizontal passes plus four half-only passes makes half-water
# flow exactly twice as often laterally while full water keeps FastFreddy's
# original cost and behavior.
replace_once(
...
# Expose the mass rules as constexpr contracts for cheap regression testing.
replace_once(
...
print("Applied current-base half-water integration.")
