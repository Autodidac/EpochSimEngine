...
required = {
    "win": ["case VK_SPACE:\n                self->jump = true", "case 'P':", "impl_->jump = key_down(VK_SPACE)"],
    ...
    "move": ["sourceCovered", "secondDrop", "swarmRest", "orbitDirection", "stabilizedRegion", "sourceWet"],
    "chem": ["countWithin(p, MAT_BEE, 6) < 12u", "setStateValue(result, 480u)",
             "randomValue & 2047u", "source.material == MAT_WASTE &&", "machineConsumption",
             "MAT_ALUMINUM_SHAVINGS) return 1", "freshContact"],
    "frag": ["sidebarWidth", "materialPixel(pixel", "3, cardMaterial", "cardPixel(pixel", "2,cardMaterial", "SPACE JUMP"],
    "reset": ["clean-water cistern", "Irrigation trench", "pollinator habitat", "MAT_CARBON_DIOXIDE", "MAT_HYDROGEN"],
    "materials": ["wetDarken"],
}
errors = []
for name, tokens in required.items():
    for token in tokens:
        if token not in files[name]: errors.append(f"{name}: missing {token}")
forbidden = {
    ...
    "chem": ["nearWater || nearFire || nearAcid || nearLava) result = makeCell(MAT_WASTE)",
}
for name, tokens in forbidden.items():
    for token in tokens:
        if token in files[name]: errors.append(f"{name}: forbidden legacy contract remains: {token}")
if errors:
    raise SystemExit("\n".join(errors))
